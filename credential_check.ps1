...
if ([IntPtr]::Size -ne 8) {
    throw "Run this from 64-bit PowerShell."
}
...
$checks = @(
    ...
    @{
        ...
        custom_check = @(
            ...
            # Affects Win10 1709+ and Server 2016 (shared codebase). Server 2016 ReleaseID is 1607 so the build gate doesn't apply to it.
            $Affected = (($OS_Name -Match "Windows 10") -And ($OS_Release -ge 1709)) -Or ($OS_Name -Match "Server 2016")
            If ((($SPN_Validation -eq 1) -Or ($SPN_Validation -eq 2)) -And $Affected) {"1"}
            Else {"0"}
            )
    }
    @{
    ...
    @{
        check_type   = 'custom'
        description  = "TCP port 135 (RPC endpoint mapper) is listening"
        info         = "Nessus uses RPC over TCP 135 to start WMI calls. If nothing is listening on 135, WMI fails."
        solution     = "Ensure the 'Remote Procedure Call (RPC)' service is running and the firewall allows inbound TCP 135."
        see_also     = "https://docs.tenable.com/nessus/Content/CredentialedChecksOnWindows.htm"
        custom_check = @(
            $listening = Get-NetTCPConnection -LocalPort 135 -State Listen -ErrorAction SilentlyContinue
            If ($listening) {"0"} Else {"1"}
            )
    }
    @{
        check_type   = 'custom'
        description  = "Administrative shares (ADMIN$, C$, IPC$) are published"
        info         = "Nessus needs the ADMIN$, C$, and IPC$ shares to read remote files and registry. The AutoShare* registry keys say they should exist; this check confirms they actually do."
        solution     = "Restart the 'Server' (LanmanServer) service, or remove any GPO/script that deletes these shares with 'net share /delete'."
        see_also     = "https://docs.tenable.com/nessus/Content/CredentialedChecksOnWindows.htm"
        custom_check = @(
